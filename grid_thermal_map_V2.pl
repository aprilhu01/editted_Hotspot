...
sub usage () {
    ...
    print("usage: grid_thermal_map.pl <lcf_file> <random_file> > <filename>.svg (or)\n");    
...
&usage() if (@ARGV != 1 && @ARGV != 2 && @ARGV != 4 && @ARGV != 6 || ! -f $ARGV[0] || ! -f $ARGV[1]);

#constants used throughout the program
	my $num_levels=21;			#number of colors used
	my $max_rotate=200; 		#maximum hue rotation
#	my @floor_map_path;#path to the file containing floorplan
	my @temp_map_path; #path to the grid temperature file 
	my @svg_output_path;#path to the output svg
	my $layer_config_path=$ARGV[0];#path to the file containing MULTIPLE floorplaN
...
#read from lcf to get names of flp
open (LCFFILE, "< $layer_config_path") || die("error: file $layer_config_path could not be opened\n");
while(<LCFFILE>){
    next if (/^\s*#|^\s*$/);
    my($line) = $_;
    chomp($line);
    ...
    push(@floor_map_path,$line);
}
close(LCFFILE);
$numfiles = @floor_map_path;

for(my $i = 0;$i<$numfiles;$i++){
    my $svg_path = substr($floor_map_path[$i],0,-3);
    $svg_path = "$svg_path"."svg";
    push(@svg_output_path,$svg_path);
}

for (my $j = 0;$j<$numfiles;$j++) {
	my $temp_path = "layer_"."$j".".grid.steady";
	push(@temp_map_path,$temp_path);
#	generate steady grid file name list
}
...
for (my $k = 0; $k<$numfiles; $k++){
	...
	my $tm;#the SVG to draw the thermalmap
	...
	open(FP, "< $floor_map_path[$k]")    or die "Couldn't open $floor_map_path[$k] for reading: $!\n";
...
#				$min_t=$6 if $6<$min_t;$max_t=$6 if $6>$max_t;
	...
	open(TM, "< $temp_map_path[$k]")    or die "Couldn't open $temp_map_path[$k] for reading: $!\n";
...
if (@ARGV == 6) {
	...
	open(FIG, "> $svg_output_path[$k]")    or die "Couldn't open $svg_output_path[$k] for reading: $!\n";	
...
SVG_FOOTER
	my $fig = $svgheader.$tm.$fp.$svgfooter;
