...
thermal_config_t default_thermal_config(void)
{
	...
	/*my changes: add nof to default thermal config*/
	config.layer_specified = 0;
	config.num_layer = 0;
...
void thermal_config_add_from_strs(thermal_config_t *config, str_pair *table, int size)
{
	...
	/*my changes: read nof from table to thermal_config*/
	if ((idx = get_str_index(table, size, "nof")) >= 0){
		char buffer[MAX_ENTRIES];
		int buf = 0;
		if(sscanf(table[idx].value, "%s", buffer)>=0){
			if(atoi(buffer)){
				buf = atoi(buffer);			//transform string to integer
				config->layer_specified = 1;
				config->layer_specified = buf;
			}
			else
				fatal("invalid format for number of layer\n");
		}
