...
void global_config_from_strs(global_config_t *config, str_pair *table, int size)
{
  ...
  /*my_changes: add section to read number of the layer to print */
  if((idx = get_str_index(table,size, "nol"))>=0) {
      if(sscanf(table[idx].value, "%s", config->num_layer) != 1)
        fatal("invalid format for configuration  parameter lc\n");
  } else {
      strcpy(config->num_layer,NULLFILE);
  }
}
...
int global_config_to_strs(global_config_t *config, str_pair *table, int max_entries)
{
  ...
  /*my changes: convert config of nof into the table */
  sprintf(table[6].name, "nof");
  sprintf(table[6].value, "%s", config->num_layer);
...
int main(int argc, char **argv)
{
   ...
   * my changes: dump all for 3D
   */
  if(model->type == GRID_MODEL &&
      strcmp(model->config->grid_steady_file, NULLFILE)&&(do_detailed_3D)&&(!model->config->layer_specified))
    dump_all_layer_temp_grid(model->grid);
  ...
  if (model->type == GRID_MODEL &&
      strcmp(model->config->grid_steady_file, NULLFILE)&&(!do_detailed_3D))
    dump_steady_temp_grid(model->grid, model->config->grid_steady_file);
