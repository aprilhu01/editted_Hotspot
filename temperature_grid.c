...
grid_model_t *alloc_grid_model(thermal_config_t *config, flp_t *flp_default, int do_detailed_3D)
{
  ...
  /*my changes: align nof to grid_model*/
  if (model->config.layer_specified)
  {
    model->layer_specified = 1;
    model->num_layer = model->config.num_layer;
  }
...
void dump_top_layer_temp_grid (grid_model_t *model, char *file, grid_model_vector_t *temp)
{
  ...
  */
  if (model->layer_specified){
    if(model->num_layer <= model->n_layers){
      silidx = model->num_layer;
    }else {
      fatal("The specified layer is out of range!\n");
    }
  }
...
void dump_all_layer_temp_grid(grid_model_t *model)
{
  ...
  {
    /* code */  
    sprintf(layer,"layer_%d.grid.steady", i);
    /*debug*/
    printf("%d:%s\n", i, layer );
    dump_x_layer_temp_grid(model, layer, model->last_steady, i);
  }
}
